...
if($foto != "") {
  ...
  if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
    move_uploaded_file($file_tmp, '../assets/images/detail/'.$nama_foto_baru); //memindah file foto ke folder foto
      // jalankan query INSERT untuk menambah data ke database pastikan sesuai urutan (id tidak perlu karena dibikin otomatis)
      $query = "INSERT INTO detail (title, content, foto) VALUES ('$title', '$content', '$nama_foto_baru')";
    ...
    echo "<script>alert('Ekstensi foto yang boleh hanya jpg, png, atau jpeg.');window.location='addd.php';</script>";
  }
} else {
  $query = "INSERT INTO detail (title, content, foto) VALUES ('$title', '$content', null)";
    $result = mysqli_query($db, $query);
    // periska query apakah ada error
    if(!$result){
      die ("Query gagal dijalankan: ".mysqli_errno($db).
